...
(function (global) {
  ...
  function ensureStyle() {
    ...
    style.textContent = [
      ...
      '  width:160px; height:120px; border-radius:20px; overflow:hidden;',
      '  border:1px solid rgba(255,255,255,.15); background:rgba(255,255,255,0.08);',
      '  backdrop-filter:blur(18px); -webkit-backdrop-filter:blur(18px);',
      '  box-shadow:0 8px 40px rgba(0,0,0,.35);',
      '  display:none; opacity:0;',
      '  transition:opacity .3s ease, backdrop-filter .3s ease, box-shadow .3s ease, transform .3s ease;',
      '}',
      '#' + DEFAULT_ID + '.hcp-visible{ display:block; opacity:1; }',
      '#' + DEFAULT_ID + ':hover{',
      '  backdrop-filter:blur(28px); -webkit-backdrop-filter:blur(28px);',
      '  box-shadow:0 14px 54px rgba(0,0,0,.5); transform:scale(1.03);',
      '}',
      '#' + DEFAULT_ID + ' video{',
      ...
      '  color:#7DD3FC; background:rgba(255,255,255,0.08); backdrop-filter:blur(8px); -webkit-backdrop-filter:blur(8px);',
